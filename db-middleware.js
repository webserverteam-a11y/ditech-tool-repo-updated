...
// --- Auth config ---
const DB_USER = process.env.DB_ADMIN_USER || 'admin';
const DB_PASS = process.env.DB_ADMIN_PASS || 'admin';
...
async function createSession() {
  ...
  try {
    await pool.query('INSERT INTO sessions (token, expires_at) VALUES (?, ?)', [token, expiresAt]);
  } catch (e) {
...
async function isValidSession(token) {
  ...
  try {
    const [rows] = await pool.query('SELECT expires_at FROM sessions WHERE token = ? LIMIT 1', [token]);
...
async function runQuery(){
  ...
  if(d.affectedRows!==undefined){
    results.innerHTML='<div class="info">Query OK. Rows affected: '+d.affectedRows+'</div>';
...
// ─── MIDDLEWARE (non-async wrapper for Express 4 compatibility) ──────────────
export default function dbMiddleware(req, res, next) {
  const pathname = (req.url || '/').split('?')[0];
  if (!pathname.startsWith('/db-access')) return next();
  _handleDbAccess(req, res).catch(err => {
    console.error('dbMiddleware error:', err);
    if (!res.headersSent) sendJson(res, { error: 'Internal server error' }, 500);
  });
}

async function _handleDbAccess(req, res) {
  const url = new URL(req.url || '/', `http://${req.headers.host || 'localhost'}`);
  const pathname = url.pathname;

  const cookies = parseCookies(req.headers.cookie);
  const authed = await isValidSession(cookies[COOKIE_NAME]);
  ...
  if (pathname === '/db-access/logout') {
    if (cookies[COOKIE_NAME]) await deleteSession(cookies[COOKIE_NAME]);
    res.writeHead(302, {
      Location: '/db-access',
  ...
  if (pathname === '/db-access/api/tables' && req.method === 'GET') {
    pool.query("SHOW TABLES").then(async ([tables]) => {
      const dbName = process.env.DB_NAME;
      const result = [];
      for (const t of tables) {
        const tableName = t[`Tables_in_${dbName}`] || Object.values(t)[0];
        const [[countRow]] = await pool.query(`SELECT COUNT(*) as c FROM \`${tableName}\``);
        result.push({ name: tableName, count: countRow.c });
      }
  ...
  if (pathname === '/db-access/api/query' && req.method === 'POST') {
    readBody(req).then(async body => {
      ...
      try {
        ...
        const isSelect = /^\s*(SELECT|SHOW|DESCRIBE|EXPLAIN|WITH)\b/i.test(trimmed);
        ...
        if (isSelect) {
          ...
          sendJson(res, { affectedRows: result.affectedRows || 0, message: 'Query executed' });
        }
      } catch (e) {
