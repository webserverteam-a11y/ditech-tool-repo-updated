...
async function initDb() {
  const ddl = [
    ...
    // history that panel needs. `client` is a plain string key everywhere
    // else in this schema (see client_config), so no FK is added here either.
  ...
  const migrations = [
    ...
    // Required by the concurrency-safe PATCH handler in backend/routes/keywords.routes.js.
    // That handler writes `updated_at = NOW()` so clients can detect stale rankings.
...
// ── DEAD CODE — this handler is permanently shadowed by the modular router
// mounted above via `app.use('/api', buildApiRouter())`. Express never reaches
// this route. The original implementation contained a destructive
// DELETE WHERE id NOT IN (...) that was the root cause of keyword data loss
// in multi-user scenarios (see backend/routes/keywords.routes.js for the fix).
// Replaced with a 410 stub so it is obviously inert if the modular router is
// ever removed.
app.put('/api/historical-keywords', (_req, res) => {
  res.status(410).json({ error: 'This endpoint has been superseded. Use PATCH /api/historical-keywords/:id to update individual keywords.' });
});
...
// ── DEAD CODE — shadowed by the modular router (app.use('/api', buildApiRouter())).
// The original implementation was a non-sparse UPDATE that overwrote ALL keyword
// fields on every edit, wiping concurrent users' changes. Fixed in the modular
// PATCH handler at backend/routes/keywords.routes.js.
app.patch('/api/historical-keywords/:id', (_req, res) => {
  res.status(410).json({ error: 'This endpoint has been superseded by the modular router.' });
});
...
app.get('/client-reports', (_req, res) => {
